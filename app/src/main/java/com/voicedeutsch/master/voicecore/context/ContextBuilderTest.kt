...
class ContextBuilderTest {
    ...
    private fun createTestSnapshot() = KnowledgeSnapshot(
        vocabulary = VocabularySnapshot(
            totalWords = 100,
            byLevel = mapOf(0 to 50, 1 to 20, 2 to 15, 3 to 10, 4 to 5),
            wordsForReviewToday = 10,
            problemWords = emptyList()
        ),
        grammar = GrammarSnapshot(
            totalRules = 20,
            byLevel = mapOf(0 to 10, 1 to 5, 2 to 3, 3 to 2),
        ...
        bookProgress = BookProgressSnapshot(
            currentChapter = 1, currentLesson = 1,
            currentTopic = "Test", completionPercentage = 0f
        ),
        recommendations = RecommendationsSnapshot(
            primaryStrategy = LearningStrategy.LINEAR_BOOK, reason = ""
        )
    )
}