...
Corrected endSession() in VoiceCoreEngineImpl.kt
The endSession method above has a syntax error in the runCatching block. Here is the correct, compilable version of that method only — replace the corresponding method in FILE 4:
override suspend fun endSession(): SessionResult? {
    ...
    // Cancel the audio loop outside the mutex to avoid deadlock
    sessionJob?.cancel()
    sessionJob = null
    ...
    return lifecycleMutex.withLock {
        ...
        val result: SessionResult? = withContext(Dispatchers.IO) {
            try {
                ...
                updateState { copy(errorMessage = "Session save failed: ${e.message}") }
                null
            }
        }
